...
import { splitSlittingProduction } from "./actions";

export default async function SplitSlittingProductionPage({
  params,
}: {
  ...
  if (!session?.user || !canManageProduction(session.user.role)) {
    redirect(`/production/slitting`);
  }

  const row = await prisma.slittingProductionData.findUnique({ where: { id } });
  if (!row) notFound();

  return (
    <div className="max-w-lg">
      <h1 className="text-xl font-semibold text-neutral-900">
        Split {row.bundleIdNo}
      </h1>
      ...
      <form
        action={splitSlittingProduction.bind(null, row.id)}

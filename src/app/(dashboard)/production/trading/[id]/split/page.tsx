...
import { splitTradingSummary } from "./actions";

export default async function SplitTradingSummaryPage({
  params,
}: {
  ...
  if (!session?.user || !canManageProduction(session.user.role)) {
    redirect(`/production/trading/${id}`);
  }

  const row = await prisma.tradingSummary.findUnique({ where: { id } });
  if (!row) notFound();

  return (
    <div className="max-w-lg">
      <h1 className="text-xl font-semibold text-neutral-900">Split {row.zsplId}</h1>
      <p className="mt-1 text-sm text-neutral-500">
        Available weight: {row.availableWeight.toString()} MT.
      </p>

      <form
        action={splitTradingSummary.bind(null, row.id)}
